...
class Etsy extends Account
{
    ...
    public function loadUserData($response)
    {
        ...
        if (!empty($result['oauth_token']) && !empty($result['oauth_token_secret'])) {
            ...
            $user = !empty($user['results'][0]['user_id'])? $user['results'][0] : [];
            $profile = !empty($profile['results'][0]['user_id'])? $profile['results'][0] : [];
