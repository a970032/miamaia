...
class Account extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function _construct()
    {
        ...
        $this->_helper = $this->_objectManager->get('Plumrocket\SocialLoginPro\Helper\Data');

        $this->_init('Plumrocket\SocialLoginPro\Model\ResourceModel\Account');
    ...
    protected function _prepareData($data)
    {
        ...
        if (! empty($_data['gender'])) {
            ...
            if ($genderAttribute && $options = $genderAttribute->getSource()->getAllOptions(false)) {
                switch ($_data['gender']) {
                    case $this->_gender[0]:
                        $_data['gender'] = $options[0]['value'];
                        break;
                    case $this->_gender[1]:
                        $_data['gender'] = $options[1]['value'];
                        break;
                    default:
                        $_data['gender'] = 0;
                }
    ...
    protected function _getRandomEmail()
    {
        ...
        $address = \Plumrocket\SocialLoginPro\Helper\Data::FAKE_EMAIL_PREFIX . $this->_objectManager->get('Magento\Framework\Math\Random')->getRandomString($len, $chars) .'@'. $domain;
        return $address;
    }
            ...
            if ($file = $this->_loadFile($fileUrl)) {
                if (file_put_contents($tmpPath, $file) > 0) {
                    ...
                    /*
                     ...
                     And copy file
                     */
                    $io->mkdir($this->_photoDir . DIRECTORY_SEPARATOR . $this->_type);
                    $io->cp(
                        $this->_photoDir . DIRECTORY_SEPARATOR . $fileName,
                        $this->_photoDir . DIRECTORY_SEPARATOR . $this->_type . DIRECTORY_SEPARATOR . $fileName
                    );
    ...
    public function getButton()
    {
        ...
        return [
            ...
            'link_text' => $this->_helper->getConfig($this->_helper->getConfigSectionId() .'/'. $this->_type .'/link_text'),
    ...
    public function getAccountImage()
    {
        ...
        if (!$photo) {
            $photo = $this->_helper->getPhotoPath(false, $this->getCustomerId());
        }
        return $photo;
    }
    ...
    protected function _getSocialModel()
    {
        ...
        $socialModel = $this->_objectManager->get('Plumrocket\SocialLoginPro\Model\\' . ucfirst($this->getType()));
